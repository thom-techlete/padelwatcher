...
class SearchOrderScheduler:
            ...
            for order in active_orders:
                try:
                    ...
                    if self._should_complete_search(order):
                        self.service.service.update_search_order_status(order.id, 'completed')
                        logger.info(f"[Order {order.id}] Search completed (date passed)")
                
                except Exception as e:
                    logger.error(f"Error processing search order {order.id}: {e}", exc_info=True)
    ...
    def get_status(self):
        ...
        return {
            ...
            'next_run_time': str(self.scheduler.get_job('search_order_job').next_run_time)
            if self.scheduler.get_job('search_order_job') else None
        }
