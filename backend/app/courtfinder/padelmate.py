...
class PadelMateService:
    ...
    def fetch_and_store_availability(
        self, location_id, date_str=None, sport_id="PADEL"
    ):
        ...
        location_obj = (
            self.service.session.query(Location)
        ...
        provider_obj = (
            self.service.session.query(Provider)
            .filter(Provider.id == location_obj.provider_id)
            .first()
        )
    ...
    def search_available_courts(
        self, date_str, start_time_range_str, end_time_range_str, duration, indoor=None
    ):
    ...
    def add_location_by_slug(
        self, slug, date_str="2025-11-13", provider_name="Playtomic"
    ):
        ...
        for _idx, court_info in enumerate(courts):
            ...
            court_obj = (
                self.service.session.query(Court)
                .filter(Court.location_id == location.id, Court.name == court_name)
                .first()
            )
            ...
            if not court_obj:
                uuid_court = (
                    self.service.session.query(Court)
                    .filter(Court.location_id == location.id, Court.name == resource_id)
                    .first()
                )
            ...
            try:
                ...
                if proper_name:
                    ...
                    if proper_court:
                        ...
                        uuid_court_avails = (
                            self.service.session.query(Availability)
                            .filter(Availability.court_id == court.id)
                            .all()
                        )
