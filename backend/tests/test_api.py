...
def test_locations(token):
    ...
    response = requests.get(
        f"{BASE_URL}/api/locations", headers={"Authorization": f"Bearer {token}"}
    )
    print(f"Status: {response.status_code}")
    data = response.json()
...
def test_add_location(token):
    ...
    response = requests.post(
        ...
        json={"slug": "padel-mate-amstelveen", "date": "2025-11-16"},
    )
    print(f"Status: {response.status_code}")
    print(f"Response: {response.json()}")
...
def test_get_search_orders(token):
    ...
    response = requests.get(
        f"{BASE_URL}/api/search-orders", headers={"Authorization": f"Bearer {token}"}
    )
    print(f"Status: {response.status_code}")
    data = response.json()
...
def run_all_tests():
    ...
    results.append(
        ("Get Search Order Results", test_get_search_order_results(token, order_id))
    )
